...
(async () => {
    ...
    await alasql("CREATE TABLE dataset (`URL ID` string, `URL beskrivelse` string, `URL adresse` string, `Dato` date, `Tid` time, `Kurs` number, `Fejl` string)");
    ...
    await alasql("INSERT INTO dataset VALUES ('XCSE05NYK01EA53', 'Nasdaq Nykredit 0,5%', 'http://www.nasdaqomxnordic.com/bonds/denmark/microsite?Instrument=XCSE05NYK01EA53', '20210423', '00:33', '95,4', '')");
    await alasql("INSERT INTO dataset VALUES ('XCSE05NYK01EA53', 'Nasdaq Nykredit 0,5%', 'http://www.nasdaqomxnordic.com/bonds/denmark/microsite?Instrument=XCSE05NYK01EA53', '20210423', '00:33', '95,4', '')");
    var res3 =await alasql("SELECT * FROM dataset");
    console.log(res3);
    var res2 = alasql('Select * INTO json("/app/data/out.json") FROM dataset');
    console.log(res2);
    debugger;
    await alasql("DELETE FROM dataset");
    var res4 =await alasql("SELECT * FROM dataset");
    console.log(res4);
    
    xxxx INSERT INTO not working!!!!
    var res5 = await alasql('SELECT * INTO dataset FROM json("/app/data/out.json")');
    console.log("Dataset:");
    console.log(res5);

    
    
    var testdata = require("/app/test-dataset");
    var res = alasql("Select * FROM ?", [testdata]);
    
    
    var res2=require("os").userInfo()
    
    
    debugger;
    var fs = require('fs');
    fs.open('/app/data/out.json', 'w', function (err, file) {
        if (err) {
            debugger;
            throw err;
        }
